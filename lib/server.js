...
app._initializeWSS = function(server) {
  ...
  this._wss.on('connection', function(socket) {
    ...
    var client = self._clients[key] && self._clients[key][id] ? self._clients[key][id] : null;
    var timeout;
    var next = (err)=>{
      clearTimeout(timeout);
      next = ()=>{}; // disallow multiple execution
      if (err) {
        socket.send(JSON.stringify({ type: 'ERROR', payload: { msg: err.toString() } }));
        socket.close();
        return;
      }
    ...
    if( client && token !== client.token && client.res){
      next("Invalid ID TAKEN");
    } else if (self._options.auth && Array.isArray(self._options.auth) && self._options.auth.some((k)=>Array.isArray(k) && k.length == 2 && id == k[0] && token == k[1])) {
      next();
    } else if(self._options.auth && Array.isArray(self._options.auth)){
      next("Invalid ID");
    } else if(self._options.auth && typeof self._options.auth == 'function'){
      timeout = setTimeout(()=>next("Auth Timeout"), 1000); // send error if auth take more then 1s
      query.ip = ip;
      try{
        self._options.auth(query,next);
      }catch(e){
        next("Error on Auth");
      }
    } else
...
app._configureWS = function(socket, key, id, token) {
  ...
  if(client.socket || token !== client.token){ //don't allow 2 connections for the same id
...
app._checkKey = function(key, ip, cb) {
  ...
  if (key == this._options.key) {
    next();
  }else if(this._options.key && typeof this._options.key == 'function'){
    timeout = setTimeout(()=>next("Key Timeout"), 1000); // send error if auth take more then 1s
    try{
      this._options.key({key:key, ip:ip},next);
    }catch(e){
      next("Error on Auth");
    }
  }else if (Array.isArray(this._options.key) && this._options.key.some((k)=>key == k)) {
    next();
  } else {
...
app._initializeHTTP = function() {
  ...
  this.get('/', function(req, res, next) {
    res.send(self._options.credits || require('../app.json'));
  });
...
app._log = function() {
  if (this._options.debug) {
    if(this._options.logger && typeof this._options.logger == 'function')
      this._options.logger.apply(null, arguments)
    else
